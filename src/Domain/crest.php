...
use const Internal\C_REST_WEB_HOOK_URL;
...
class CRest
{
	...
	protected static function callCurl($arParams)
	{
		...
		{
			return [
				'error' => 'error_php_lib_curl', 'error_information' => 'need install curl lib'
			];
		}
		...
		if ($arSettings !== false)
		{
			if (isset($arParams['this_auth']) && $arParams['this_auth'] == 'Y')
			{
				...
				$url = 'https://bx-oauth.yurta2.bx/oauth/token/';
			}
			else
			{
			...
			try
			{
				...
				static::setLog([
					'url' => $url, 'info' => $info, 'params' => $arParams, 'result' => $result
				], 'callCurl');
		...
		return [
			'error' => 'no_install_app', 'error_information' => 'error install app, pls install local application '
		];
	}
	...
	public static function callBatch($arData, $halt = 0)
	{
		...
		if (is_array($arData))
		{
			...
			if (!empty($arDataRest))
			{
				...
				$arPost = [
					'method' => 'batch', 'params' => $arDataRest
				];
	...
	private static function getAppSettings()
	{
		...
		{
			$arData = [
				'client_endpoint' => C_REST_WEB_HOOK_URL, 'is_web_hook' => 'Y'
			];
			$isCurrData = true;
		}
		else
		{
			...
			$isCurrData = false;
			if (!empty($arData['access_token']) && !empty($arData['domain']) && !empty($arData['refresh_token']) && !empty($arData['application_token']) && !empty($arData['client_endpoint']))
			{
				$isCurrData = true;
			}
		}
